...
class FrameProcessor(BaseObject):
    ...
    async def stop_ttfb_metrics(self):
        ...
        if self.can_generate_metrics() and self.metrics_enabled:
            ...
            if frame:
                await self.push_frame(frame) # type: ignore
    ...
    def create_task(
        ...
        *,
        ...
        if name:
            ...
            name = f"{self}::{coroutine.cr_code.co_name}" # type: ignore
            ...
            if direction == FrameDirection.DOWNSTREAM and self._next:
                ...
                if self._observer:
                    data = FramePushed(
                        ...
                        destination=self._next, # type: ignore
                ...
                if self._observer:
                    data = FramePushed(
                        source=self, # type: ignore
                        destination=self._prev, # type: ignore
                        frame=frame, # type: ignore
                        direction=direction, # type: ignore
                        timestamp=timestamp,
                    )
