...
from minichain.pipecat.audio.interruptions.base_interruption_strategy import BaseInterruptionStrategy
from minichain.pipecat.clocks.base_clock import BaseClock
from minichain.pipecat.clocks.system_clock import SystemClock
from minichain.pipecat.frames.frames import (
    BotSpeakingFrame,
    CancelFrame,
    CancelTaskFrame,
    EndFrame,
...
from minichain.pipecat.metrics.metrics import ProcessingMetricsData, TTFBMetricsData
from minichain.pipecat.observers.base_observer import BaseObserver
from minichain.pipecat.observers.turn_tracking_observer import TurnTrackingObserver
from minichain.pipecat.pipeline.base_pipeline import BasePipeline
from minichain.pipecat.pipeline.base_task import BasePipelineTask, PipelineTaskParams
from minichain.pipecat.pipeline.task_observer import TaskObserver
from minichain.pipecat.processors.frame_processor import FrameDirection, FrameProcessor, FrameProcessorSetup
from minichain.pipecat.utils.asyncio.task_manager import (
    WATCHDOG_TIMEOUT,
    BaseTaskManager,
    TaskManager,
    TaskManagerParams,
)
from minichain.pipecat.utils.asyncio.watchdog_queue import WatchdogQueue
from minichain.pipecat.utils.tracing.setup import is_tracing_available
from minichain.pipecat.utils.tracing.turn_trace_observer import TurnTraceObserver
...
class PipelineTaskSource(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection): # type: ignore
        ...
        """
        await super().process_frame(frame, direction) # type: ignore

        match direction:
            case FrameDirection.UPSTREAM:
                await self._up_queue.put(frame)
            case FrameDirection.DOWNSTREAM:
                await self.push_frame(frame, direction) # type: ignore
...
class PipelineTask(BasePipelineTask):
    ...
    async def queue_frame(self, frame: Frame): # type: ignore
    ...
    async def queue_frames(self, frames: Iterable[Frame] | AsyncIterable[Frame]):# type: ignore
    ...
    async def _cancel(self):
        ...
        if not self._cancelled:
            ...
            await self._source.push_frame(CancelFrame()) # type: ignore
    ...
    async def _setup(self, params: PipelineTaskParams):
        ...
        setup = FrameProcessorSetup(
            ...
            task_manager=self._task_manager, # type: ignore
    ...
    async def _cleanup(self, cleanup_pipeline: bool):
        ...
        if self._enable_tracing and hasattr(self, "_turn_trace_observer"):
            self._turn_trace_observer.end_conversation_tracing() # type: ignore
    ...
    async def _process_push_queue(self):
        ...
        await self._source.queue_frame(start_frame, FrameDirection.DOWNSTREAM) # type: ignore

        if self._params.enable_metrics and self._params.send_initial_empty_metrics:
            await self._source.queue_frame(self._initial_metrics_frame(), FrameDirection.DOWNSTREAM) # type: ignore
    ...
    async def _heartbeat_push_handler(self):
        ...
        while True:
            ...
            await self._source.queue_frame(HeartbeatFrame(timestamp=self._clock.get_time())) # type: ignore
        ...
        while running:
            try:
                ...
                if isinstance(frame, StartFrame) or isinstance(frame, self._idle_timeout_frames):
                    ...
                    if diff_time >= self._idle_timeout_secs: # type: ignore
