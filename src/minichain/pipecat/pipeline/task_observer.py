...
from minichain.pipecat.utils.asyncio.task_manager import BaseTaskManager
...
class TaskObserver(BaseObserver):
    ...
    def add_observer(self, observer: BaseObserver):
        ...
        if self._started():
            ...
            self._proxies[observer] = proxy # type: ignore
    ...
    async def remove_observer(self, observer: BaseObserver):
        ...
        if observer in self._proxies: # type: ignore
            proxy = self._proxies[observer] # type: ignore
            # Remove the proxy so it doesn't get called anymore.
            del self._proxies[observer] # type: ignore
    ...
    async def on_push_frame(self, data: FramePushed):
        ...
        """
        for proxy in self._proxies.values(): # type: ignore
        ...
        while True:
            ...
            if on_push_frame_deprecated:
                await observer.on_push_frame(
                    data.src, data.dst, data.frame, data.direction, data.timestamp # type: ignore
                )
            else:
